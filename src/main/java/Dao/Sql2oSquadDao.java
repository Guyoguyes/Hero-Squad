...
public class Sql2oSquadDao implements SquadDao {
    ...
    public void add(Squad squad) {
        String sql = "INSERT INTO squads (name, size, source) VALUES (:name, :size, :source)";
        try(Connection con = sql2o.open()){
    ...
    @Override
    public void update(int id, String name, int size, String source) {
      String sql = "UPDATE squads SET name = :name, size = :size, source = :source WHERE id=:id";
      try(Connection con = sql2o.open()){
          con.createQuery(sql)
